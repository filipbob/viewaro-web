import type { Dictionary } from "../types";

const pt: Dictionary = {
  nav: {
    features: "Funcionalidades",
    faq: "Perguntas frequentes",
    support: "Suporte",
    comingSoon: "macOS 15+",
  },
  footer: {
    tagline: "O teu conteúdo. O teu ecrã.",
    features: "Funcionalidades",
    privacy: "Política de privacidade",
    support: "Suporte",
    terms: "Termos de utilização",
    copyright: "© 2026 ITQuotes. Todos os direitos reservados.",
    madeBy: "Um produto itQuotes — itquotes.hr",
  },
  home: {
    metaTitle: "Viewaro — TV em direto, feito bem",
    metaDescription:
      "O Viewaro é um leitor IPTV nativo para macOS 15+. Importa a tua playlist M3U/M3U8 e reproduz streams a que tens acesso autorizado.",
    eyebrow: "Viewaro Player",
    heroLine1: "O teu conteúdo.",
    heroLine2: "O teu ecrã.",
    heroSub:
      "O Viewaro é um leitor IPTV nativo para Mac. Importa a tua playlist M3U/M3U8 e reproduz streams a que tens acesso autorizado. O Viewaro não fornece canais, playlists ou subscrições de conteúdos multimédia.",
    badge: "Versão atual: macOS 15+",
    plannedPlatforms:
      "Planeado: iPhone, iPad, Apple TV, Android e Android TV. Estas plataformas não estão disponíveis na versão atual para macOS.",
    featuresEyebrow: "Funcionalidades",
    featuresHeading: "Tudo o que uma app de TV em direto deve ser.",
    features: [
      {
        title: "TV em direto e guia",
        body: "Um guia de programação integrado com progresso em direto e o que vem a seguir — sem precisar de um ecrã separado.",
      },
      {
        title: "A tua playlist M3U",
        body: "Importa uma playlist M3U ou M3U8 compatível de um fornecedor que estás autorizado a usar.",
      },
      {
        title: "Favoritos e pesquisa",
        body: "Cria uma lista de canais pessoal e encontra qualquer coisa instantaneamente, em todas as fontes que adicionaste.",
      },
      {
        title: "Dados locais",
        body: "Playlists, credenciais, favoritos e definições ficam no teu Mac. Não existe conta Viewaro nem sincronização na nuvem.",
      },
      {
        title: "Controlo parental",
        body: "Um PIN de quatro dígitos protege a gestão de perfis e a saída de um perfil infantil; as categorias restritas ficam ocultas.",
      },
      {
        title: "Feito para Mac",
        body: "Uma experiência nativa para macOS 15+ com controlos Mac, navegação por teclado e armazenamento local.",
      },
    ],
    integrationsEyebrow: "No roteiro",
    integrationsHeading: "Filmes e séries",
    integrationsSub: "Funcionalidade planeada — não disponível no macOS 1.0.",
    integrations: [
      {
        name: "TMDB",
        body: "Cartazes, elenco e descrições para o catálogo de filmes e séries.",
      },
      {
        name: "Trakt",
        body: "Sincronização de histórico e progresso de visualização, caso já uses lá.",
      },
    ],
    plannedLabel: "Planeado",
    howEyebrow: "Como funciona",
    howHeading: "Pronto num minuto.",
    steps: [
      {
        step: "01",
        title: "Instala o Viewaro",
        body: "Instala a versão atual num Mac com macOS 15 ou posterior.",
      },
      {
        step: "02",
        title: "Adiciona a tua playlist",
        body: "Cola o endereço da tua playlist M3U ou M3U8. Os canais importados aparecem em segundos.",
      },
      {
        step: "03",
        title: "Começa a ver",
        body: "TV em direto com guia de programação completo, favoritos e mudança de canal instantânea. É tudo.",
      },
    ],
    faqEyebrow: "Perguntas frequentes",
    faqHeading: "Boas perguntas.",
    faqs: [
      {
        q: "O Viewaro inclui canais ou streams?",
        a: "Não. O Viewaro é um leitor — não contém canais, streams ou subscrições de qualquer tipo. Ligas a tua própria playlist do teu próprio fornecedor, e o Viewaro dá-lhe um lar bonito.",
      },
      {
        q: "Que plataformas são suportadas?",
        a: "A versão atual suporta Macs com macOS 15 ou posterior. iPhone, iPad, Apple TV, Android e Android TV estão planeados, mas não disponíveis no macOS 1.0.",
      },
      {
        q: "Que formatos de playlist posso usar?",
        a: "A versão atual importa playlists M3U e M3U8. Também podes indicar um endereço de guia XMLTV opcional. Contas Xtream Codes e introdução manual de canais não estão disponíveis no macOS 1.0.",
      },
      {
        q: "Preciso de uma conta?",
        a: "Não. O macOS 1.0 não tem conta Viewaro, início de sessão ou sincronização na nuvem. Playlists, favoritos, perfis e definições ficam no teu Mac.",
      },
      {
        q: "Como funciona o Viewaro Premium?",
        a: "O Viewaro Premium é uma subscrição Apple na app, renovável automaticamente, por 4,99 € por mês. Desbloqueia várias fontes IPTV e perfis. A Apple mostra o preço local antes da compra e gere faturação, cancelamento e reembolsos.",
      },
    ],
    ctaHeading: "Versão atual: macOS 1.0",
    ctaBody:
      "Para macOS 15 e posterior. Tu forneces a tua playlist M3U/M3U8; o Viewaro não oferece canais nem subscrições multimédia.",
  },
  legal: {
    updatedLabel: "Data de entrada em vigor:",
    privacy: {
      title: "Política de privacidade",
      metaDescription: "Como o Viewaro trata os teus dados.",
      updated: "25 de setembro de 2026",
      sections: [
        {
          paragraphs: [
            "O Viewaro é fornecido pela **IT QUOTES, vl. Marina Blažon Bobinac** (\"Viewaro\", \"nós\" ou \"nosso\"). Esta Política de Privacidade explica como a app Viewaro para macOS e o site de suporte Viewaro tratam a informação.",
          ],
        },
        {
          list: [
            "Email: [support@itquotes.hr](mailto:support@itquotes.hr)",
            "Morada postal: Vukomerečka cesta 15, 10000 Zagreb, Croácia",
          ],
        },
        {
          heading: "Resumo",
          paragraphs: [
            "O Viewaro é um leitor IPTV onde trazes o teu próprio conteúdo. Não fornece canais, catálogos de filmes ou séries, playlists, contas de fornecedor ou subscrições de media. Os dados das fontes e as preferências de reprodução são guardados localmente no teu Mac. Não usamos rastreadores publicitários e não vendemos dados pessoais.",
            "Se as subscrições estiverem ativadas na versão que utilizas, a Apple processa o pagamento e a RevenueCat processa informação limitada de compra e técnica para validar a compra e determinar se as funcionalidades Premium estão disponíveis.",
          ],
        },
        {
          heading: "O que o Viewaro guarda no teu Mac",
          paragraphs: ["O Viewaro pode guardar localmente o seguinte:"],
        },
        {
          list: [
            "nomes das fontes e os endereços de playlist, de fornecedores Xtream e de servidores Tvheadend que introduzes",
            "credenciais ou tokens contidos nos endereços de playlist, e os nomes de utilizador e palavras-passe que introduzes para fornecedores Xtream e servidores Tvheadend",
            "informação do catálogo importado de canais, filmes, séries, temporadas e episódios, com os endereços de imagens e as descrições que o teu fornecedor disponibiliza",
            "dados do guia de TV transferidos e qualquer endereço de guia que introduzas",
            "favoritos, progresso de visualização, definições da app e preferências relacionadas com a reprodução",
            "um identificador de subscrição anónimo e o estado de direito em cache quando as subscrições estão ativadas",
          ],
        },
        {
          paragraphs: [
            "Os endereços de playlist sensíveis, as contas de fornecedores e servidores e os endereços de guia são guardados num cofre local encriptado. A chave do cofre é protegida pelo Chaveiro (Keychain) do macOS. Esta informação não é enviada para servidores da Viewaro ou da ITQuotes.",
            "Podes remover fontes importadas a partir da app. O macOS pode reter itens do Chaveiro após a remoção de uma aplicação, pelo que desinstalar a app não é um método garantido de apagar todos os itens do Chaveiro.",
          ],
        },
        {
          heading: "Registo do sistema do macOS",
          paragraphs: [
            "O macOS pode gravar no registo do sistema do Mac os endereços dos streams que o Viewaro abre. São os próprios componentes de rede da Apple, e não o Viewaro, que escrevem estas entradas, e o Viewaro não as pode remover. Os fornecedores Xtream colocam o teu nome de utilizador e a tua palavra-passe em cada endereço de stream, e algumas playlists colocam um token ou uma palavra-passe nos seus endereços; nessas fontes, o endereço registado contém esses dados da conta.",
            "O registo do sistema permanece no teu Mac. Qualquer pessoa com uma conta de administrador nesse Mac pode lê-lo, e é incluído nos relatórios de diagnóstico que escolheres criar e partilhar, por exemplo com a Apple. O Viewaro não nos envia o registo do sistema.",
          ],
        },
        {
          heading: "Ligações de rede ao teu fornecedor",
          paragraphs: [
            "Quando importas uma fonte, carregas as suas imagens ou o seu guia, ou reproduzes um stream, o teu Mac liga-se diretamente ao endereço que forneceste e aos endereços de imagens que esse fornecedor devolve. Esses hosts podem receber informação normalmente incluída numa ligação de rede, como o teu endereço IP, a hora do pedido, o caminho solicitado e quaisquer credenciais que o pedido ao fornecedor exija.",
            "Um servidor Tvheadend recebe a conta que introduziste para ele: o Viewaro inicia sessão quando o servidor o solicita e envia a conta com cada canal que reproduzes. Se o servidor estiver na tua rede local, o macOS pede-te autorização antes de o Viewaro se poder ligar a ele.",
            "Não controlamos o teu fornecedor nem recebemos esses pedidos. Aplicam-se a política de privacidade e os termos do teu fornecedor. O Viewaro dá preferência a HTTPS. Quando um endereço do fornecedor usa HTTP, o Viewaro avisa que a ligação pode expor credenciais e tenta primeiro o mesmo host por HTTPS. Se o fornecedor não suportar HTTPS, o Viewaro pode usar o endereço HTTP original não encriptado para uma playlist, guia, imagem ou stream fornecidos pelo utilizador. Um servidor Tvheadend é usado no endereço exatamente como o introduzes, muitas vezes um endereço http:// numa rede doméstica; num endereço assim, a palavra-passe que introduziste é enviada sem encriptação sempre que um canal é reproduzido, e o Viewaro avisa disso antes da importação. Usa HTTPS sempre que o teu fornecedor ou servidor o disponibilizar.",
          ],
        },
        {
          heading: "Fontes do guia de TV",
          paragraphs: ["A origem do guia depende da fonte:"],
        },
        {
          list: [
            "**Um endereço de guia que introduziste, ou o endereço de guia XMLTV que a própria playlist inclui:** O Viewaro liga-se apenas a esse endereço, tal como ao teu fornecedor.",
            "**Um servidor Tvheadend:** O Viewaro transfere o guia do próprio servidor com a conta que introduziste e não contacta nenhum editor de guias públicos para esse servidor.",
            "**Uma playlist sem endereço de guia próprio, ou um fornecedor Xtream:** O Viewaro procura automaticamente um guia público na primeira utilização da fonte e quando atualizas o guia. Enquanto a app estiver aberta, volta também a procurar quando o guia transferido tiver mais de 12 horas ou estiver a chegar ao fim, ou quando uma tentativa anterior não tiver encontrado nenhum. Solicita ficheiros XMLTV por país a open-epg.com e iptv-epg.org. O Viewaro não pede ao fornecedor Xtream o seu guia.",
          ],
        },
        {
          paragraphs: [
            "Estes editores recebem dados de rede normais, como o teu endereço IP, a hora do pedido e o ficheiro do país solicitado. O Viewaro não lhes envia o endereço do fornecedor, credenciais, nomes de canais ou histórico de visualização. A correspondência dos programas ocorre localmente no teu Mac; aplicam-se a política de privacidade e os termos de cada editor.",
          ],
        },
        {
          heading: "Demo HLS da Apple",
          paragraphs: [
            "A demonstração de reprodução integrada liga-se diretamente a uma amostra HLS pública alojada pela Apple. A Apple pode receber informação padrão de pedidos de rede, como o teu endereço IP e a hora do pedido, nos termos de privacidade da Apple.",
          ],
        },
        {
          heading: "Compras e RevenueCat",
          paragraphs: [
            "As compras são feitas através do sistema de compras integradas da Apple. Nunca recebemos os dados do teu cartão de pagamento ou conta bancária.",
            "Quando as subscrições estão ativadas, o Viewaro usa a RevenueCat para validar recibos da Apple e fornecer o estado de direito Premium. A RevenueCat pode processar:",
          ],
        },
        {
          list: [
            "um ID de utilizador de app anónimo gerado aleatoriamente",
            "o recibo da Apple e o histórico de compras ou subscrições",
            "tipo de dispositivo, informação do sistema operativo, versão da app e hora da última atividade",
          ],
        },
        {
          paragraphs: [
            "Esta informação é usada para validação de compras, prevenção de fraude, funcionalidade de subscrição e análise agregada de subscrições. O Viewaro não envia endereços de fontes, credenciais de fornecedor, nomes de canais ou histórico de visualização para a RevenueCat. Não usamos dados da RevenueCat para rastreamento publicitário entre apps.",
            "Consulta a [Política de Privacidade da RevenueCat](https://www.revenuecat.com/privacy) para as suas práticas de processamento, retenção e transferência internacional.",
          ],
        },
        {
          heading: "Comunicações de suporte",
          paragraphs: [
            "Se contactares o suporte, recebemos a informação que escolheres fornecer, como o teu endereço de email, mensagem, versão da app e detalhes de diagnóstico. Nunca envies URLs de playlist, nomes de utilizador, palavras-passe, tokens ou outras credenciais de fornecedor.",
            "Usamos a informação de suporte para responder ao teu pedido, investigar problemas e cumprir obrigações legais. Retemo-la apenas pelo tempo razoavelmente necessário para esses fins.",
          ],
        },
        {
          heading: "Análise, publicidade e rastreamento",
          paragraphs: [
            "O Viewaro não inclui SDKs publicitários nem análise comportamental na versão atual. Não rastreamos a tua atividade entre apps ou sites e não vendemos dados pessoais.",
            "A informação de compra processada pela RevenueCat é usada apenas conforme descrito na secção Compras e RevenueCat acima. Esta política e a declaração de privacidade da App Store serão atualizadas antes de ativar qualquer análise ou SDK de terceiros materialmente diferente.",
          ],
        },
        {
          heading: "Partilha de dados",
          paragraphs: ["Só partilhamos informação quando necessário:"],
        },
        {
          list: [
            "com a Apple para processar compras e distribuir a app",
            "com a RevenueCat para validar compras e gerir direitos de subscrição",
            "com fornecedores de serviços usados para operar o site de suporte público ou o email",
            "quando exigido por lei ou necessário para proteger utilizadores, os nossos direitos ou o público",
          ],
        },
        {
          paragraphs: [
            "Não autorizamos fornecedores de serviços a usar informação do Viewaro para a sua própria publicidade.",
          ],
        },
        {
          heading: "As tuas escolhas e direitos",
          paragraphs: [
            "Podes remover fontes e outros dados locais da app através dos controlos do Viewaro. Podes gerir ou cancelar uma subscrição Apple nas definições da tua conta Apple. Podes contactar-nos para perguntar sobre informação de suporte que temos sobre ti ou para exercer direitos disponíveis ao abrigo da lei de privacidade aplicável.",
            "Como os dados das fontes e de reprodução permanecem no teu Mac, geralmente não conseguimos aceder, exportar ou eliminar esses dados locais remotamente.",
          ],
        },
        {
          heading: "Crianças",
          paragraphs: [
            "O Viewaro não é dirigido a crianças e não recolhe conscientemente informação pessoal de crianças. Os utilizadores são responsáveis por garantir que o conteúdo a que acedem é apropriado e legal.",
          ],
        },
        {
          heading: "Segurança",
          paragraphs: [
            "Usamos medidas técnicas concebidas para proteger informação, incluindo a App Sandbox do macOS, a proteção do Chaveiro e armazenamento local encriptado para endereços de playlist e segredos de contas. Nenhuma medida de segurança pode garantir proteção absoluta.",
          ],
        },
        {
          heading: "Alterações",
          paragraphs: [
            "Atualizaremos esta política à medida que a funcionalidade do Viewaro, os nossos fornecedores de serviços ou obrigações legais mudarem. A versão atual e a data de entrada em vigor serão publicadas nesta página.",
          ],
        },
        {
          heading: "Contacto",
          paragraphs: [
            "Perguntas ou pedidos sobre privacidade podem ser enviados para [support@itquotes.hr](mailto:support@itquotes.hr) ou para IT QUOTES, vl. Marina Blažon Bobinac, Vukomerečka cesta 15, 10000 Zagreb, Croácia.",
          ],
        },
      ],
    },
    terms: {
      title: "Termos de utilização",
      metaDescription: "Termos de utilização da app Viewaro.",
      updated: "24 de setembro de 2026",
      sections: [
        {
          paragraphs: [
            "Estes Termos de Utilização (\"Termos\") aplicam-se à app Viewaro para macOS e ao site de suporte relacionado, fornecidos pela **IT QUOTES, vl. Marina Blažon Bobinac** (\"Viewaro\", \"nós\" ou \"nosso\").",
          ],
        },
        {
          heading: "Termos de licença da Apple",
          paragraphs: [
            "A app é licenciada através da Mac App Store, não vendida. A menos que um contrato de licença de utilizador final personalizado seja apresentado na App Store Connect, o [EULA padrão da Apple](https://www.apple.com/legal/internet-services/itunes/dev/stdeula/) rege a licença da app. Estes Termos complementam o EULA padrão da Apple para serviços específicos do Viewaro e uso aceitável. Os direitos obrigatórios do consumidor no teu país permanecem inalterados.",
          ],
        },
        {
          heading: "Traz o teu próprio conteúdo",
          paragraphs: [
            "O Viewaro é um leitor multimédia. Não fornece, aloja, vende, recomenda nem verifica canais de televisão, catálogos de filmes ou séries, playlists, subscrições de media ou credenciais de acesso.",
            "Só podes usar o Viewaro com fontes e conteúdo aos quais estejas legalmente autorizado a aceder. És responsável por:",
          ],
        },
        {
          list: [
            "a legalidade e exatidão dos endereços de playlist, fornecedor e servidor e das credenciais que introduzes",
            "obter todas as subscrições, licenças e permissões necessárias",
            "cumprir os termos do teu fornecedor e a lei aplicável",
            "garantir que o conteúdo é apropriado para cada pessoa que usa o teu Mac",
          ],
        },
        {
          paragraphs: [
            "Não deves usar o Viewaro para infringir direitos de autor, contornar controlos de acesso ou DRM, obter acesso não autorizado a um serviço, partilhar credenciais roubadas ou facilitar a distribuição ilegal.",
          ],
        },
        {
          heading: "Fornecedores e serviços de terceiros",
          paragraphs: [
            "O Viewaro liga-se diretamente a fornecedores selecionados por ti. Não controlamos esses fornecedores nem garantimos a sua disponibilidade, segurança, legalidade, compatibilidade, conteúdo ou desempenho. A tua relação com cada fornecedor é regida pelos seus próprios termos e política de privacidade.",
            "A Demo HLS da Apple é uma amostra pública usada apenas para demonstrar a reprodução. Não é um serviço de conteúdo do Viewaro.",
          ],
        },
        {
          heading: "Funcionalidades gratuitas e Premium",
          paragraphs: [
            "O Viewaro pode oferecer um conjunto de funcionalidades gratuitas e funcionalidades Premium opcionais através de uma subscrição de renovação automática. Antes da compra, a app mostrará:",
          ],
        },
        {
          list: [
            "a funcionalidade Premium incluída",
            "a duração da subscrição",
            "o preço localizado",
            "se se aplica uma oferta experimental ou introdutória",
            "que a subscrição se renova automaticamente até ser cancelada",
          ],
        },
        {
          paragraphs: [
            "O pagamento é cobrado à tua conta Apple após confirmação. Renovação, faturação, reembolsos e cancelamento são geridos pela Apple segundo os seus termos aplicáveis. Podes gerir ou cancelar a subscrição nas definições da tua conta Apple. Eliminar o Viewaro não cancela automaticamente uma subscrição.",
            "O acesso Premium depende de uma compra e direito válidos da Apple. O Viewaro fornece uma ação Restaurar Compras para compras elegíveis. Preços, durações e ofertas apresentados pela App Store prevalecem se diferirem do texto do site.",
          ],
        },
        {
          heading: "Atualizações e disponibilidade",
          paragraphs: [
            "Podemos atualizar o Viewaro para adicionar, alterar ou remover funcionalidades, manter a segurança, cumprir a lei ou permanecer compatível com o macOS e serviços de terceiros. Não garantimos que todos os fornecedores, formatos de playlist, codecs ou streams funcionem.",
            "Podemos suspender o acesso a serviços online específicos do Viewaro quando necessário por motivos de segurança, manutenção, conformidade legal ou prevenção de abusos. Isto não altera os direitos que a Apple concede durante um período de subscrição ativo.",
          ],
        },
        {
          heading: "Privacidade e segurança",
          paragraphs: [
            "O nosso tratamento da informação está descrito na [Política de Privacidade do Viewaro](/pt/privacy). Deves manter as credenciais do fornecedor confidenciais e evitar incluí-las em mensagens de suporte.",
          ],
        },
        {
          heading: "Isenção de garantia e limitação de responsabilidade",
          paragraphs: [
            'O EULA padrão da Apple contém os termos de garantia e responsabilidade aplicáveis à licença da app. Na máxima medida permitida pela lei aplicável, o Viewaro é fornecido "tal como está" e "conforme disponível". Não somos responsáveis por conteúdo de terceiros, interrupções do fornecedor, alterações à playlist ou uso não autorizado de fontes introduzidas por um utilizador.',
            "Nada nestes Termos exclui ou limita responsabilidade que não possa ser legalmente excluída, nem limita direitos obrigatórios do consumidor.",
          ],
        },
        {
          heading: "Rescisão",
          paragraphs: [
            "Podes deixar de usar o Viewaro a qualquer momento. Os teus direitos ao abrigo destes Termos terminam se os violares materialmente, sujeito à lei aplicável e ao EULA padrão da Apple. As secções que pela sua natureza devam sobreviver à rescisão permanecem em vigor, incluindo disposições de propriedade intelectual, isenção de garantia e responsabilidade.",
          ],
        },
        {
          heading: "Alterações a estes Termos",
          paragraphs: [
            "Podemos atualizar estes Termos quando o Viewaro, a lei aplicável ou os nossos fornecedores de serviços mudarem. A versão atual e a data de entrada em vigor serão publicadas nesta página. Alterações materiais serão comunicadas quando exigido por lei.",
          ],
        },
        {
          heading: "Contacto",
          paragraphs: [
            "Perguntas sobre estes Termos podem ser enviadas para [support@itquotes.hr](mailto:support@itquotes.hr) ou para IT QUOTES, vl. Marina Blažon Bobinac, Vukomerečka cesta 15, 10000 Zagreb, Croácia.",
          ],
        },
      ],
    },
    support: {
      title: "Suporte",
      metaDescription: "Obtém ajuda com o Viewaro.",
      sections: [
        {
          paragraphs: [
            "O Viewaro 1.0 é um leitor IPTV nativo para Macs com macOS 15 ou posterior. Importa as playlists M3U/M3U8 que forneces e reproduz streams compatíveis. Não existe conta de utilizador nem sincronização na nuvem; o Viewaro não inclui canais, playlists ou subscrições multimédia.",
          ],
        },
        {
          heading: "Contacto",
          list: [
            "Email de suporte: [support@itquotes.hr](mailto:support@itquotes.hr)",
            "Fornecedor: IT QUOTES, vl. Marina Blažon Bobinac",
            "Morada postal: Vukomerečka cesta 15, 10000 Zagreb, Croácia",
          ],
        },
        {
          paragraphs: ["Ao contactar o suporte, inclui:"],
        },
        {
          list: [
            "a tua versão do Viewaro",
            "a tua versão do macOS",
            "o passo que falhou",
            "a mensagem de erro exata, se alguma foi mostrada",
          ],
        },
        {
          paragraphs: [
            "Nunca envies URLs de playlist, nomes de utilizador, palavras-passe, tokens, capturas de ecrã com credenciais ou outros segredos do fornecedor.",
          ],
        },
        {
          heading: "Perguntas comuns",
          topics: [
            {
              q: "Onde consigo uma playlist?",
              a: "O Viewaro não vende nem recomenda serviços IPTV. Pede a um fornecedor a cuja utilização estás legalmente autorizado um endereço de playlist M3U compatível.",
            },
            {
              q: "Posso usar uma playlist ou um stream HTTP?",
              a: "Sim, se o endereço pertencer ao fornecedor que escolheste. O Viewaro avisa quando um endereço introduzido do fornecedor usa HTTP e tenta primeiro o mesmo host por HTTPS. Se o fornecedor suportar apenas HTTP, a aplicação pode voltar ao endereço original. Os URLs de playlists e streams contêm frequentemente credenciais, pelo que uma ligação HTTP pode expô-las na rede. Usa HTTPS sempre que o teu fornecedor o disponibilizar.",
            },
            {
              q: "Como verifico se a reprodução funciona?",
              a: "Abre a TV em direto e usa a Demo HLS da Apple. Reproduz uma amostra pública da Apple sem exigir credenciais de fornecedor.",
            },
            {
              q: "O que inclui a versão gratuita?",
              a: "A versão gratuita suporta uma fonte IPTV, um perfil e reprodução completa. O Viewaro Premium custa 4,99 € por mês e desbloqueia várias fontes IPTV e perfis. A App Store mostra o preço localizado antes da compra.",
            },
            {
              q: "Como restauro ou cancelo o Premium?",
              a: "Abre Definições do Viewaro > Subscrição e escolhe Restaurar Compras. Usa Gerir Subscrição, quando disponível, ou as definições de subscrições da tua conta Apple para gerir ou cancelar. Eliminar o Viewaro não cancela a subscrição.",
            },
            {
              q: "Como funcionam o controlo parental e o PIN?",
              a: "Abre Definições do Viewaro > Controlo parental para definir, alterar ou desativar um PIN de quatro dígitos. O PIN protege a gestão de perfis e a saída de um perfil infantil. Se o esqueceres, o Viewaro não o pode repor na app. Fecha e elimina o Viewaro; depois, no Finder, escolhe Ir > Ir para a pasta e move ~/Library/Containers/hr.itquotes.viewaro para o Lixo antes de reinstalar. Isto elimina permanentemente playlists, credenciais, favoritos, perfis e definições locais; não cancela o Premium.",
            },
            {
              q: "O Viewaro envia a minha playlist?",
              a: "O Viewaro guarda a informação da playlist localmente no teu Mac e liga-se diretamente ao endereço do fornecedor que forneceste. Consulta a [Política de Privacidade](/pt/privacy) para mais detalhes.",
            },
          ],
        },
        {
          heading: "Legal",
          list: [
            "[Política de Privacidade](/pt/privacy)",
            "[Termos de Utilização](/pt/terms)",
            "[EULA padrão da Apple](https://www.apple.com/legal/internet-services/itunes/dev/stdeula/)",
          ],
        },
      ],
    },
  },
};

export default pt;
